...
class Node(ModelBase):
    ...
    def object_permissions(self):
        if self._node_permissions is None:
            self._node_permissions = self.permissions.all()
        return self._node_permissions

    @property
    def user_permissions(self):
        return filter(lambda perm: perm.object_ct == ContentType.objects.get_for_model(User), self.object_permissions)

    @property
    def group_permissions(self):
        return filter(lambda perm: perm.object_ct == ContentType.objects.get_for_model(Group), self.object_permissions)
